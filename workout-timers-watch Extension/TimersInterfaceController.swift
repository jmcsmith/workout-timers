...
class TimersInterfaceController: WKInterfaceController {
    ...
    override func awake(withContext context: Any?) {
        ...
        if let workout = context as? Workout {
            ...
            for iterator in rows..<timersTable.numberOfRows {
                // 1
                let rowController = timersTable.rowController(at: iterator)
                // 2
                if let controller = rowController as? TimerRowController {
                    controller.timerName.setText(workout.timers[iterator - 1].name)
                    controller.timerTime.setText(workout.timers[iterator - 1].time.description)
                    // 3
                }
            }
        }
    ...
    func setTimerToIndex(index: Int) {
        ...
        timer = UIKit.Timer.scheduledTimer(timeInterval: 1, target: self, selector: (#selector(updateTimer)),
